...
void Window::paintFPS()
{
	...
	painter.drawText(10, 20, QString("FPS:%1").arg(QString::number(mFps, 'f', 3)));
}

void Window::initShaders()
{
	if (!program.addShaderFromSourceFile(QOpenGLShader::Vertex, "res/shader/vertex.vsh"))
	{
		qDebug() << __FILE__ << __FUNCTION__ << " add vertex shader file failed.";
		return;
	}
	if (!program.addShaderFromSourceFile(QOpenGLShader::Fragment, "res/shader/fragment.fsh"))
	{
