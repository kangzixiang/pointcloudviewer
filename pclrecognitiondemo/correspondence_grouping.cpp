...
void showHelp(char *filename) {
  ...
  std::cout << "     -c:                     Show used correspondences."
            << std::endl;
  std::cout << "     -r:                     Compute the model cloud "
               "resolution and multiply"
            << std::endl;
  ...
  std::cout << "     --algorithm (Hough|GC): Clustering algorithm used "
               "(default Hough)."
            << std::endl;
  std::cout << "     --model_ss val:         Model uniform sampling radius "
               "(default 0.01)"
            << std::endl;
  std::cout << "     --scene_ss val:         Scene uniform sampling radius "
               "(default 0.03)"
            << std::endl;
  std::cout
  ...
  std::cout << "     --descr_rad val:        Descriptor radius (default 0.02)"
            << std::endl;
  std::cout << "     --cg_size val:          Cluster size (default 0.01)"
            << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  for (std::size_t i = 0; i < scene_descriptors->size(); ++i) {
    ...
    if (found_neighs == 1 &&
        neigh_sqr_dists[0] <
            0.25f) //  add match only if the squared descriptor distance is less
                   //  than 0.25 (SHOT descriptor distances are between 0 and 1
                   //  by design)
    {
  ...
  if (show_keypoints_) {
    ...
    pcl::visualization::PointCloudColorHandlerCustom<PointType>
        off_scene_model_keypoints_color_handler(off_scene_model_keypoints, 0, 0,
                                                255);
  ...
  for (std::size_t i = 0; i < rototranslations.size(); ++i) {
    ...
    if (show_correspondences_) {
      for (std::size_t j = 0; j < clustered_corrs[i].size(); ++j) {
        ...
        //  We are drawing a line for each pair of clustered correspondences
        //  found between the model and the scene
        viewer.addLine<PointType, PointType>(model_point, scene_point, 0, 255,
                                             0, ss_line.str());
      }
    }
  }
