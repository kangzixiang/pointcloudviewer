...
void Window::initDatas()
{
	...
	poitLightPositions << QVector3D(0.7f, 0.2f, 2.0f) << QVector3D(2.3f, -3.3f, -4.0f) << QVector3D(-4.0f, 2.0f, -12.0f) << QVector3D(0.0f, 0.0f, -3.0f);
}
...
void Window::paintGL()
{
...

	{
		...
		}
		{
			...
			for (int i = 0; i < 4; i++)
			{
				program.setUniformValue(QString("pointLights[%1].ambient").arg(i).toStdString().data(), lightAmibent);
				program.setUniformValue(QString("pointLights[%1].diffuse").arg(i).toStdString().data(), lightDiffuse);
				program.setUniformValue(QString("pointLights[%1].specular").arg(i).toStdString().data(), lightSpecular);
				program.setUniformValue(QString("pointLights[%1].position").arg(i).toStdString().data(), poitLightPositions[i]);

				program.setUniformValue(QString("pointLights[%1].constant").arg(i).toStdString().data(), (GLfloat)1.0f);
				program.setUniformValue(QString("pointLights[%1].linear").arg(i).toStdString().data(), (GLfloat)0.09);
				program.setUniformValue(QString("pointLights[%1].quadratic").arg(i).toStdString().data(), (GLfloat)0.032);
			}
		}
		{
			// Spot light
			...
			program.setUniformValue("spotLight.cutoff", (GLfloat)cos(qDegreesToRadians(6.)));
			program.setUniformValue("spotLight.outerCutOff", (GLfloat)cos(qDegreesToRadians(9.)));
