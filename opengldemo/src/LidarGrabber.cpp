...
CLidarGrabber::CLidarGrabber(std::string pcapFile) : m_pcapFile(pcapFile) {
...
CLidarGrabber::CLidarGrabber(std::string pcapFile, std::string ipAddress,
                             int port)
...
int CLidarGrabber::lidarGrabberInit() {
  ...
  sweep_xyzi_signal_ =
      new boost::signals2::signal<sig_sweep_point_cloud_xyzi>();
  ...
  if (!m_pcapFile.empty()) {
    ...
    if (bind(m_sockfd, reinterpret_cast<sockaddr *>(&my_addr),
             sizeof(sockaddr)) == -1) {
...
void CLidarGrabber::start() {
  ...
  case READMODE::READFROMPCAP: {
    m_readPacketThreadHandle =
        new std::thread(&CLidarGrabber::readLidarPacketFromPcap, this);
    break;
  }
  case READMODE::READFROMSOCKET: {
    m_readPacketThreadHandle =
        new std::thread(&CLidarGrabber::readLidarPacketFromSocket, this);
    break;
  }
  default:
    break;
  ...
  m_processPacketThreadHandle =
      new std::thread(&CLidarGrabber::processLidarPackets, this);
}
...
bool CLidarGrabber::isRunning() const { return m_readPacketThread; }
...
void CLidarGrabber::readLidarPacketFromPcap() {
  ...
  pcap_t *pcap =
      pcap_open_offline(m_pcapFile.c_str(), reinterpret_cast<char *>(errbuff));
  ...
  if (pcap_compile(pcap, &filter, "udp", 0, 0xffffffff) == -1) {
    printf("WARN--[CLidarGrabber::readPacketsFromPcap] Issue compiling filter: "
           "%s.\n",
           pcap_geterr(pcap));
  } else if (pcap_setfilter(pcap, &filter) == -1) {
    printf("WARN--[CLidarGrabber::readPacketsFromPcap] Issue setting filter: "
           "%s.\n",
           pcap_geterr(pcap));
  }
  ...
  while (returnValue >= 0) {
    ...
    std::uint64_t usec_delay =
        ((header->ts.tv_sec - lasttime.tv_sec) * 1000000) +
...
void CLidarGrabber::readLidarPacketFromSocket() {
  ...
  while (true) {
    ...
    printf("INFO--Incomplete Velodyne rising data packet read: %zd bytes from "
           "port %d",
           nbytes, m_port);
  }
...
void CLidarGrabber::enqueueLidarPacket(const std::uint8_t *data,
                                       std::size_t bytesReceived) {
  if (bytesReceived == 1206) {
    uint8_t *dup = static_cast<std::uint8_t *>(
        malloc(bytesReceived * sizeof(std::uint8_t)));
...
void CLidarGrabber::toPointClouds(LaserDataPacket *dataPacket) {
  ...
  for (int i = 0; i < LASER_FIRING_PER_PKT; i++) {
    double azimuthInRadians = HDL_Grabber_toRadians(
        (static_cast<double>(dataPacket->firingData[i].rotationalPosition) /
         100.0));
    printf("\nblockIdentifier: %x\n",
    ...
    std::uint8_t offset =
        (dataPacket->firingData[i].blockIdentifier == BLOCK_0_TO_31) ? 0 : 32;
    for (int j = 0; j < LASER_PER_FIRING; j++) {
      printf(" -distance:%x",
             dataPacket->firingData[i].laserReturns[j].distance);
      printf(" -intensity:%x",
             dataPacket->firingData[i].laserReturns[j].intensity);
      PointXYZI point;
      computeXYZI(point, dataPacket->firingData[i].rotationalPosition,
                  dataPacket->firingData[i].laserReturns[j],
                  m_laser_corrections[j + offset]);
      printf(" -point.x:%f, point.y:%f, point.z:%f, point.i:%f\n", point.x,
             point.y, point.z, point.i);
    }
  }
  printf("\ngpsTimestamp: %x\n", gpsTimestamp);
  printf("mode: %x\n", mode);
...
void CLidarGrabber::toPointClouds(std::uint8_t *data) {
  ...
  for (int i = 0; i < LASER_FIRING_PER_PKT; i++) {
    ...
    // blockIdentifier[1], firingData.blockIdentifier);
    // printf("rotationalPosition: %x%x(%x)\n", rotationalPosition[0],
    // rotationalPosition[1], firingData.rotationalPosition);
    ...
    for (int j = 0; j < LASER_PER_FIRING; j++) {
      ...
      // printf(" -distance:%x%x(%x)", distance[0], distance[1],
      // laserReturns.distance); printf(" -intensity:%x(%x)", intensity,
      // laserReturns.intensity);
      firingData.laserReturns[j] = laserReturns;
      PointXYZI point;
      computeXYZI(point, firingData.rotationalPosition,
                  firingData.laserReturns[j], m_laser_corrections[j]);
      // printf(" -point.x:%f, point.y:%f, point.z:%f, point.i:%f\n", point.x,
      // point.y, point.z, point.i);
      m_vecCurrentPacketMutex.lock();
  ...
  // printf("\ngpsTimestamp: %x%x%x%x(%x)\n", gpsTimestamp[0], gpsTimestamp[1],
  // gpsTimestamp[2], gpsTimestamp[3], lidarPacket.gpsTimestamp); printf("mode:
  // %x(%x)\n", mode, lidarPacket.mode); printf("sensorType: %x(%x)\n",
...
bool CLidarGrabber::socketAvailable(int timeout) {
  ...
  do {
    ...
    if (retval < 0) { // poll() error?
      if (errno != EINTR) {
        printf("WARN--Velodyne port %d poll() error: %s", m_port,
               strerror(errno));
      }
      return false;
    }
...
void CLidarGrabber::loadHDL32Corrections() {
  double hdl32_vertical_corrections[] = {
      -30.67,     -9.3299999, -29.33,    -8,     -28,        -6.6700001, -26.67,
      -5.3299999, -25.33,     -4,        -24,    -2.6700001, -22.67,     -1.33,
      -21.33,     0,          -20,       1.33,   -18.67,     2.6700001,  -17.33,
      4,          -16,        5.3299999, -14.67, 6.6700001,  -13.33,     8,
      -12,        9.3299999,  -10.67,    10.67};
  for (std::uint8_t i = 0; i < LASER_PER_FIRING; i++) {
    ...
    m_laser_corrections[i].sinVertCorrection =
        std::sin(HDL_Grabber_toRadians(hdl32_vertical_corrections[i]));
    m_laser_corrections[i].cosVertCorrection =
...
void CLidarGrabber::computeXYZI(PointXYZI &point, std::uint16_t azimuth,
  ...
  point.x =
      static_cast<float>(xyDistance * sin_azimuth -
                         correction.horizontalOffsetCorrection * cos_azimuth);
  point.y =
      static_cast<float>(xyDistance * cos_azimuth +
